...
export class ActorSheetPFNPCLoot extends ActorSheetPFNPC {
  get template() {
    ...
    Handlebars.registerHelper('lootsheetweight', function(baseWeight, count) {
      let weightConversion = game.settings.get("warcraftrpg2e", "units") === "metric" ? 0.5 : 1;
    ...
    const path = "systems/warcraftrpg2e/templates/actors/";
    return "systems/warcraftrpg2e/templates/actors/npc-sheet-loot.html";
  }
  ...
  async getData() {
    ...
    sheetData.weightUnit = game.settings.get("warcraftrpg2e", "units") == "metric" ? game.i18n.localize("D35E.Kgs") : game.i18n.localize("D35E.Lbs")
  ...
  async activateListeners(html) {
    ...
    if (this.options.editable) {
      root.querySelectorAll('.permission-proficiency').forEach(el => el.addEventListener("click", ev => this._onCyclePermissionProficiency(ev)));
      root.querySelectorAll('.permission-batch').forEach(el => el.addEventListener("click", ev => this._onBatchPermissionChange(ev)));
      root.querySelectorAll('.split-coins').forEach(el => el.addEventListener("click", ev => this._distributeCoins(ev)));
      root.querySelectorAll('.price-modifier').forEach(el => el.addEventListener("click", ev => this._priceModifier(ev)));
      root.querySelectorAll('.identify-all').forEach(el => el.addEventListener("click", ev => this._identifyAll(ev)));
      root.querySelectorAll('.group-items').forEach(el => el.addEventListener("click", ev => this._groupItems(ev)));
      root.querySelectorAll('.unidentify-all').forEach(el => el.addEventListener("click", ev => this._unidentifyAll(ev)));
      root.querySelectorAll('.convert-loot').forEach(el => el.addEventListener("click", ev => this._convertLoot(ev)));
      root.querySelectorAll('.update-inventory').forEach(el => el.addEventListener("click", ev => this._merchantInventoryUpdate(ev)));
    }

    root.querySelectorAll('.item-buy').forEach(el => el.addEventListener("click", ev => this._buyItem(ev)));
    root.querySelectorAll('.item-sell').forEach(el => el.addEventListener("click", ev => this._sellItem(ev)));
    root.querySelectorAll('.item-loot').forEach(el => el.addEventListener("click", ev => this._lootItem(ev)));
    root.querySelectorAll('.item-visibility').forEach(el => el.addEventListener("click", ev => this._toggleVisibility(ev)));
  }
  ...
  async _merchantInventoryUpdate(event, html) {
    ...
    let clearInventory = game.settings.get("warcraftrpg2e", "clearInventory");
  ...
  _createRollTable() {
    ...
    const pack = game.packs.find(p => p.collection === "warcraftrpg2e.items");
  ...
  _buyItem(event) {
    ...
    if (game.user.character) {
      let itemEl = event.currentTarget.closest(".item");
      let itemId = itemEl.getAttribute("data-item-id");
      let quantity = Number(itemEl.getAttribute("data-item-quantity"));
      let itemName = itemEl.querySelector("h4").textContent;
  ...
  _sellItem(event) {
    ...
    if (game.user.character) {
      let itemEl = event.currentTarget.closest(".item");
      let itemId = itemEl.getAttribute("data-item-id");
      let maxQuantity = Number(itemEl.getAttribute("data-item-quantity"));
      let itemName = itemEl.querySelector("h4").textContent;
  ...
  _lootItem(event) {
    ...
    if (game.user.character) {
      let itemEl = event.currentTarget.closest(".item");
      let itemId = itemEl.getAttribute("data-item-id");
      const isCurrency = itemId.length === 2 || itemId.startsWith("wl_");

      if (isCurrency) {
        const currKey = itemId.startsWith("wl_") ? itemId.substring(3) : itemId;
        const currObj = itemId.startsWith("wl_") ? this.actor.system.altCurrency : this.actor.system.currency;
        const available = currObj?.[currKey] ?? 0;
        const currencyLabel = game.i18n.localize("D35E.ls." + itemId);
        let options = {
          acceptLabel: game.i18n.localize("D35E.ls.loot"),
          title: game.i18n.format("D35E.ls.lootTitle", { item: currencyLabel }),
        };
        if (available === 1) {
          options.label = game.i18n.format("D35E.ls.lootContent", { item: currencyLabel });
          options.quantity = 1;
        }
        let d = new QuantityDialog(async (quantity) => {
          await LootSheetActions.moveCoins(this.actor, game.user.character, itemId, quantity);
          this.render(true);
        }, options);
        d.render(true);
      } else {
        ...
        let d = new QuantityDialog(async (quantity) => {
          await LootSheetActions.moveItem(this.actor, game.user.character, itemId, quantity);
          this.render(true);
        }, options);
        d.render(true);
      }
  ...
  async _identifyAll(event) {
    ...
    this.actor.items.forEach( item  => {
          if (["weapon", "equipment", "consumable", "tool", "loot"].indexOf(item.type) >= 0) {
            updateList.push({ id: item.id, 'system.identified': true })
          }
        }
    );
  ...
  async _unidentifyAll(event) {
    ...
    this.actor.items.forEach( item  => {
          if (["weapon", "equipment", "consumable", "tool", "loot"].indexOf(item.type) >= 0) {
            updateList.push({ id: item.id, 'system.identified': false })
          }
        }
    );
    await this.actor.updateEmbeddedDocuments("Item", updateList)
  ...
  async _priceModifier(event) {
    ...
    foundry.applications.handlebars.renderTemplate("systems/warcraftrpg2e/templates/lootsheet/dialog-price-modifier.html", {'priceModifier': priceModifier, 'priceModifierBuy': priceModifierBuy}).then(html => {
      new Dialog({
  ...
  _toggleVisibility(event) {
    event.preventDefault();
    let itemId = event.currentTarget.closest(".item").getAttribute("data-item-id");
  ...
  async _convertLoot(event) {
    ...
    Dialog.confirm({
      ...
      yes: () => (async () => {
        ...
        let funds = foundry.utils.duplicate(this.actor.system.currency)
  ...
  async _distributeCoins(event) {
    ...
    // Calculate owners
    for (let u in actorData.ownership) {
      if (u != "default" && actorData.ownership[u] == 2) {
        //game.D35E.logger.log("Loot Sheet | u in actorData.ownership", u);
    ...
    let currencySplit = foundry.utils.duplicate(actorData.currency);
    let altCurrencySplit = foundry.utils.duplicate(actorData.altCurrency);
    let currencyRemains = foundry.utils.duplicate(actorData.currency);
    let altCurrencyRemains = foundry.utils.duplicate(actorData.altCurrency);
    ...
    for (let u of owners) {
      ...
      let newCurrency = foundry.utils.duplicate(u.system.currency);
      let newAltCurrency = foundry.utils.duplicate(u.system.altCurrency);
      ...
      if (msg.length != 0) {
        let message = game.i18n.format("D35E.ls.receives", {actor: u.name});
        message += msg.join(",");
        ChatMessage.create({
          user: game.user.id,
          speaker: {
  ...
  async _onCyclePermissionProficiency(event) {
    ...
    let field = event.currentTarget.parentElement.querySelector('input[type="hidden"]');

    let level = parseFloat(field.value);
    ...
    let playerId = field.name;

    let permissions = foundry.utils.duplicate(this.actor.ownership)
  ...
  async _onBatchPermissionChange(event) {
    event.preventDefault();
    let newLevel = Number(event.currentTarget.getAttribute("data-perm"))
    let permissions = foundry.utils.duplicate(this.actor.ownership)
    game.users.forEach((u) => {
  ...
  _prepareItems(actorData) {
    ...
    const itemGroups = {
      ...
      tools: {
        label: game.i18n.localize("D35E.ls.tools"),
        items: [],
        type: "tool"
      },
      containers: {
        label: game.i18n.localize("D35E.ls.containers"),
        items: [],
        type: "container"
      },
    };
  ...
  #splitItemsToGroups(actorData, itemGroups = null) {
    ...
    for (let i of actorData.items) {
      ...
      else if (i.type === "valuable") itemGroups.valuables.items.push(i);
  ...
  _prepareGMSettings(actorData) {
    ...
    //game.D35E.logger.log("Loot Sheet _prepareGMSettings | actorData.ownership", actorData.ownership);
      ...
      if (u.role === 1 || u.role === 2) {
        ...
        if (actor) {
          ...
          if (typeof actorData.ownership.default !== "undefined") {
            ...
            game.D35E.logger.log("Loot Sheet | default permissions", actorData.ownership.default);

            u.lootPermission = actorData.ownership.default;

            if (actorData.ownership.default === 2 && !owners.includes(actor.id)) {
          ...
          if (u.id in actorData.ownership && !owners.includes(actor.id)) {
            game.D35E.logger.log("Loot Sheet | Found individual actor permission");

            u.lootPermission = actorData.ownership[u.id];
            game.D35E.logger.log("Loot Sheet | assigning " + actorData.ownership[u.id] + " permission to hidden field");

            if (actorData.ownership[u.id] === 3) {
              owners.push(actor.id);
            }
          }
    ...
    let currencySplit = foundry.utils.duplicate(actorData.system.currency);
    let altCurrencySplit = foundry.utils.duplicate(actorData.system.altCurrency);
    for (let c in currencySplit) {
      if (owners.length) {
  ...
  async _onDropItem(event) {
    ...
    if (item.actor)
      LootSheetActions.dropOrSellItem(item.actor, targetActor, item.actor, item.id)
