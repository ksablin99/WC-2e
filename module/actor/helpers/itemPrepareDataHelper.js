    ...
    static prepareResistancesForItem(item, erDrRollData, actorPrepareData) {
        ...
        if (item.system.resistances) {
            (item.system?.resistances || []).forEach(resistance => {
                ...
                let _oldResistance = foundry.utils.duplicate(_resistance);
                // Fix up existing objects so they work
                erDrRollData.level = item.system.levels || 0;
                erDrRollData.levels = item.system.levels || 0;

                _resistance.value = Math.max(_resistance.value, new Roll35e(resistance[0] || "0", erDrRollData).evaluateSync().total);
        ...
        if (item.system.damageReduction) {
            (item.system?.damageReduction || []).forEach(dr => {
                ...
                if (dr[1] !== 'any') {
                    ...
                    let _oldDr = foundry.utils.duplicate(dr);
                    erDrRollData.level = item.system.levels || 0;
                    erDrRollData.levels = item.system.levels || 0;
                    _dr.value = Math.max(_dr.value, Roll35e.safeEvaluate(dr[0] || "0", erDrRollData).total);
                    ...
                    actorPrepareData.combinedDR.any = Math.max(actorPrepareData.combinedDR.any || 0, new Roll35e(dr[0] || "0", erDrRollData).evaluateSync().total);
                }
            });
        }
                    ...
                    if (enhancementItemData.damageReduction) {
                        (enhancementItemData?.damageReduction || []).forEach(dr => {
                            ...
                            if (dr[1] !== 'any') {
                                ...
                                erDrRollData.levels = enhancementItemData.levels || 0;
                                erDrRollData.enh = enhancementItemData.enh || 0;
                                _dr.value = Math.max(_dr.value, new Roll35e(dr[0] || "0", erDrRollData).evaluateSync().total);
                                _dr.immunity = _dr.immunity || dr[2];
                                if (!isEqual(_oldDr,_dr)) {
                                    _dr.providedBy = item.id;
                                    _dr.modified = true;
                                ...
                                actorPrepareData.combinedDR.any = Math.max(actorPrepareData.combinedDR.any || 0, new Roll35e(dr[0] || "0", enhancementItemData).evaluateSync().total);
                            }
                        });
                    }
                });
