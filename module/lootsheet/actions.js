...
export class LootSheetActions {
  ...
  static async moveItem(source, destination, itemId, quantity = 1) {
    ...
    if (update["system.quantity"] === 0) {
      ...
      await source.updateEmbeddedDocuments("Item", [update]);
    }

    newItem.system.quantity = quantity;
    await destination.createEmbeddedDocuments("Item", [newItem], { keepWeight: true });
  ...
  static async moveCoins(source, destination, itemId, quantity) {
    if (!source || !destination) {
      ui.notifications.error(game.i18n.localize("ERROR.lsNoActiveCharacter"));
      return null;
    }
    if (itemId.startsWith("wl_")) {
      ...
      // Move all items if we select more than the quantity.
      let coins = source.system.altCurrency[itemId] ?? 0;
      if (coins < quantity) {
        quantity = coins;
      }
      ...
      srcUpdate.system.altCurrency[itemId] = (source.system.altCurrency[itemId] ?? 0) - quantity;
      await source.update(srcUpdate);

      const dstUpdate = { system: { altCurrency: {} } };
      dstUpdate.system.altCurrency[itemId] = (destination.system.altCurrency[itemId] ?? 0) + quantity;
      await destination.update(dstUpdate);
    } else {
      // Move all items if we select more than the quantity.
      let coins = source.system.currency[itemId] ?? 0;
      if (coins < quantity) {
        quantity = coins;
      }
      ...
      srcUpdate.system.currency[itemId] = (source.system.currency[itemId] ?? 0) - quantity;
      await source.update(srcUpdate);

      const dstUpdate = { system: { currency: {} } };
      dstUpdate.system.currency[itemId] = (destination.system.currency[itemId] ?? 0) + quantity;
      await destination.update(dstUpdate);
    }
  ...
  static async transaction(
    ...
    let sellerFunds = foundry.utils.duplicate(seller.system.currency);
