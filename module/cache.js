...
export const buildCache = async function() {
    ...
    const additionalCachedCompendiums_classAbilities = game.settings.get("warcraftrpg2e", "additionalCachedCompendiums_classAbilities");
    const additionalCachedCompendiums_racialAbilities = game.settings.get("warcraftrpg2e", "additionalCachedCompendiums_racialAbilities");
    const additionalCachedCompendiums_spellLikeAbilities = game.settings.get("warcraftrpg2e", "additionalCachedCompendiums_spellLikeAbilities");
    const additionalCachedCompendiums_materials = game.settings.get("warcraftrpg2e", "additionalCachedCompendiums_materials");
    const additionalCachedCompendiums_damageTypes = game.settings.get("warcraftrpg2e", "additionalCachedCompendiums_damageTypes");

    for (const entry of game.packs.entries()) {
        const packName = entry[0];
        const itemPack = entry[1];

        if (isClassAbilityPack(packName, additionalCachedCompendiums_classAbilities)) {
            await addClassAbilitiesFromPackToCache(itemPack);
            continue;
        }
