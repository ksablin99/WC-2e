 ...
 */
export class TemplateLayerPF extends foundry.canvas.layers.TemplateLayer {
  // Foundry does not respect CONFIG.MeasuredTemplate.documentClass and CONFIG.MeasuredTemplate.objectClass
  async _onDragLeftStart(event) {
    if (!game.settings.get("warcraftrpg2e", "measureStyle")) return super._onDragLeftStart(event);

    // Call placeables layer super instead of templatelayer
    const origin = foundry.utils.duplicate(event.interactionData.origin);
    await foundry.canvas.layers.PlaceablesLayer.prototype._onDragLeftStart.call(this, event);
    ...
    // Snap to grid
    if (!event.shiftKey) {
      const pos = this.getSnappedPoint(origin);
      origin.x = pos.x;
      origin.y = pos.y;
    }
  ...
  _onDragLeftMove(event) {
    if (!game.settings.get("warcraftrpg2e", "measureStyle")) return super._onDragLeftMove(event);

    let { destination } = event.interactionData;
    const { layerDragState, preview, origin } = event.interactionData;
    if (layerDragState === 0) return;

    // Snap the destination to the grid
    const snapToGrid = !event.shiftKey;
    if (snapToGrid) {
      destination = event.interactionData.destination = this.getSnappedPoint(destination);
    }
    ...
    if (snapToGrid && ["cone", "circle"].includes(type)) {
      ...
      preview.document.updateSource({ direction: Math.floor((baseDirection + halfAngle / 2) / halfAngle) * halfAngle });
    } else if (snapToGrid && type === "ray") {
      preview.document.updateSource({ direction: Math.floor((baseDirection + cellSize / 2) / cellSize) * cellSize });
    } else {
      preview.document.updateSource({ direction: baseDirection });
    }
    // Set distance
    const baseDistance = ray.distance / ratio;
    if (snapToGrid && ["cone", "circle", "ray"].includes(type)) {
      preview.document.updateSource({ distance: Math.floor(baseDistance / dist) * dist });
    } else {
      preview.document.updateSource({ distance: baseDistance });
    }
...
export class MeasuredTemplatePF extends foundry.canvas.placeables.MeasuredTemplate {
  static get defaults() {
        return {
          angle: 90.0,
          width: 1
  ...
  getHighlightedSquares() {
    if (!game.settings.get("warcraftrpg2e", "measureStyle") || !["circle", "cone", "ray"].includes(this.document.t)) return [];

    const grid = canvas.grid,
      gridSizePx = canvas.dimensions.size, // Size of each cell in pixels
      gridWidthPx = grid.sizeX ?? grid.w ?? gridSizePx,
      gridHeightPx = grid.sizeY ?? grid.h ?? gridSizePx,
    ...
    if (templateType === "ray") {
      ...
      const ray = foundry.canvas.geometry.Ray.fromAngle(this.ray.A.x, this.ray.A.y, this.ray.angle, this.ray.distance + gridSizePx / 2);
    ...
    const nr = Math.ceil((this.document.distance * 1.5) / gridSizeUnits / (gridSizePx / gridHeightPx)),
      nc = Math.ceil((this.document.distance * 1.5) / gridSizeUnits / (gridSizePx / gridWidthPx));
  ...
  getTokensWithin() {
    ...
    if (canvas.grid.type === CONST.GRID_TYPES.GRIDLESS && ["circle", "cone", "rect"].includes(shape)) {
      ...
      for (const t of canvas.tokens.placeables) {
        switch (shape) {
          case "circle": {
            const ray = new foundry.canvas.geometry.Ray(this.center, t.center);
  ...
  highlightGrid() {
    if (
      !game.settings.get("warcraftrpg2e", "measureStyle") ||
    ...
    const highlightSquares = this.getHighlightedSquares();
    for (const s of highlightSquares) {
      canvas.interface.grid.highlightPosition(this.highlightId, { x: s.x, y: s.y, color: fc, border: bc });
    }
  }
  ...
  `
			{
				...
				if (this.shape.width && this.shape.height)
					...
					const rays = angles.map(a => foundry.canvas.geometry.Ray.fromAngle(0, 0, direction + toRadians(a), distance+1));
...
foundry.canvas.placeables.MeasuredTemplate.prototype.refresh = Function(`"use strict"; return ( function ${newFun} )`)();
