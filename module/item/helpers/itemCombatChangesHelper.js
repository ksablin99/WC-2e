    ...
    items
      ...
      .forEach((i) => {
        ...
        if (
          ...
          optionalFeatIds.indexOf(i.id) !== -1
        ) {
          allCombatChanges = allCombatChanges.concat(
            i.combatChanges.getPossibleCombatChanges(attackType + "Optional", rollData, optionalFeatRanges.get(i.id))
          );

          if (optionalFeatRanges.get(i.id)) {
            let ranges = [];
            if (optionalFeatRanges.get(i.id).base) ranges.push(optionalFeatRanges.get(i.id).base);
            if (optionalFeatRanges.get(i.id).slider1) ranges.push(optionalFeatRanges.get(i.id).slider1);
            if (optionalFeatRanges.get(i.id).slider2) ranges.push(optionalFeatRanges.get(i.id).slider2);
            if (optionalFeatRanges.get(i.id).slider3) ranges.push(optionalFeatRanges.get(i.id).slider3);
          ...
          i.addCharges(
            -1 *
              (i.system.combatChangesUsesCost === "chargesPerUse"
                ...
                : optionalFeatRanges.get(i.id).base)
          );
        }
      });
