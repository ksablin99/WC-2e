...
export class ItemEnhancementConverter {
    static async toEnhancement(origData, type, cl) {
        ...
        let newItemData = foundry.utils.duplicate(itemTemplate?.enhancement ?? {});
        for (const t of newItemData.templates ?? []) {
            foundry.utils.mergeObject(newItemData, foundry.utils.duplicate(itemTemplate?.templates?.[t] ?? {}));
        }
        ...
        if (system.enhancementRequirements === undefined) system.enhancementRequirements = "";
