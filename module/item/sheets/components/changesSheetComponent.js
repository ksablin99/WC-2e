...
export class ChangesSheetComponent extends ItemSheetComponent {
  registerTab(sheetData) {
    sheetData.registeredTabs.push({
      id: "changes",
      name: "Changes",
      sheet: "systems/warcraftrpg2e/templates/items/parts/item-changes.html",
    });
  }

  activateListeners(html) {
    const root = html?.nodeType === 1 ? html : html?.[0] ?? html;
    root.querySelectorAll(".change-control").forEach(el => el.addEventListener("click", this.#onChangeControl.bind(this)));
    root.querySelectorAll(".combat-change-control").forEach(el => el.addEventListener("click", this.#onCombatChangeControl.bind(this)));
    root.querySelectorAll(".requirement-control").forEach(el => el.addEventListener("click", this.#onRequirementsControl.bind(this)));
    root.querySelectorAll(".creation-changes-control").forEach(el => el.addEventListener("click", this.#onCreationChangesControl.bind(this)));
    root.querySelectorAll(".resistance-control").forEach(el => el.addEventListener("click", this.#onResistanceControl.bind(this)));
    root.querySelectorAll(".dr-control").forEach(el => el.addEventListener("click", this.#onDRControl.bind(this)));
  }
  ...
  prepareSheetData(sheetData) {
    ...
    if (this.sheet.item.system.changes) {
      const localizedModifiers = Object.fromEntries(
        Object.entries(CONFIG.D35E.bonusModifiers).map(([k, v]) => [k, game.i18n.localize(v)])
      );
      sheetData.changes = { targets: {}, modifiers: localizedModifiers };
      ...
      sheetData.data.system.changes.forEach((item) => {
        ...
        // Add slot subtargets (one per tracked slot from defaultSlotCapacities)
        else if (item[1] === "slot") {
          for (const key of Object.keys(CONFIG.D35E.defaultSlotCapacities)) {
            const label = game.i18n.localize(`D35E.EquipSlot${key.charAt(0).toUpperCase()}${key.slice(1)}`);
            item.subTargets[`slot.${key}`] = label;
          }
        }
        // Add static targets
  ...
  async #onCombatChangeControl(event) {
    ...
    if (a.classList.contains("delete-change")) {
      ...
      const changes = foundry.utils.duplicate(this.sheet.item.system.combatChanges);
  ...
  async #onCreationChangesControl(event) {
    ...
    if (a.classList.contains("add-change")) {
      //await this._onSubmit(event);  // Submit any unsaved changes
      const changes = foundry.utils.duplicate(this.sheet.item.system.creationChanges) || [];
      // Combat Changes are
  ...
  async #onRequirementsControl(event) {
    ...
    if (a.classList.contains("add-change")) {
      //await this._onSubmit(event);  // Submit any unsaved changes
      const changes = foundry.utils.duplicate(this.sheet.item.system.requirements) || [];
      // Combat Changes are
  ...
  async #onResistanceControl(event) {
    ...
    if (a.classList.contains("add-change")) {
      //await this._onSubmit(event);  // Submit any unsaved changes
      const changes = foundry.utils.duplicate(this.sheet.item.system.resistances) || [];
      // Combat Changes are
  ...
  async #onDRControl(event) {
    ...
    if (a.classList.contains("add-change")) {
      //await this._onSubmit(event);  // Submit any unsaved changes
      const changes = foundry.utils.duplicate(this.sheet.item.system.damageReduction) || [];
      // Combat Changes are
  ...
  async #onChangeControl(event) {
    ...
    // Add new change
    if (a.classList.contains("add-change")) {
      let _changes = foundry.utils.duplicate(this.sheet.item.system.changes) || [];
    ...
    if (a.classList.contains("delete-change")) {
      const li = a.closest(".change");
      const changes = foundry.utils.duplicate(this.sheet.item.system.changes);
      changes.splice(Number(li.dataset.change), 1);
