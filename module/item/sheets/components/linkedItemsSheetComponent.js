...
export class LinkedItemsSheetComponent extends ItemSheetComponent {
  ...
  registerTab(sheetData) {
    sheetData.registeredTabs.push({
      id: "links",
      name: "Links",
      sheet: "systems/warcraftrpg2e/templates/items/parts/item-links.html",
    });
  }

  activateListeners(html) {
    const root = html?.nodeType === 1 ? html : html?.[0] ?? html;
    root.querySelectorAll('div[data-tab="links"]').forEach(el => el.addEventListener("drop", this.#onDrop.bind(this, "link")));
    root.querySelectorAll('div[data-tab="links"] .item-delete').forEach(el => el.addEventListener("click", this.#onLinkedItemDelete.bind(this)));
  }
  ...
  async #onLinkedItemDelete(event) {
    ...
    if (game.keyboard.isModifierActive("Shift")) {
      const updateData = {};
      let _linkedItems = foundry.utils.duplicate(foundry.utils.getProperty(this.sheet.item.system, `linkedItems`) || []);
  ...
  async #onDrop(importType, event) {
    ...
    if (droppedData.type === "Item") {
      ...
      // Case 3 - Resolved Item document (not compendium — linked tab still requires compendium in #importItem)
      else if (droppedData instanceof Item) {
        dataType = droppedData.isEmbedded ? "data" : "world";
        itemData = droppedData.toObject(false);
      }

      else {
        dataType = "world";
        itemData = fromUuidSync(droppedData.uuid)?.toObject?.(false) ?? {};
      }
