...
export class ItemChatData {
  ...
  async getChatData(htmlOptions, rollData) {
    ...
    const itemChatData = foundry.utils.duplicate(this.item.system);
    ...
    itemChatData.description.value = await foundry.applications.ux.TextEditor.enrichHTML(
    ...
    if (!this.item.showUnidentifiedData) {
      ...
      if (itemChatData.spellDurationData != null) {
        ...
        if (
          ...
          if (rollString.indexOf("@cl") !== -1) {
            duration = new Roll35e(rollString, rollData).evaluateSync().total;
            let multiplier = 0;
      ...
      if (
        ...
        (foundry.utils.getProperty(itemChatData, "save.type") ||
          (foundry.utils.getProperty(itemChatData, "save.description") &&
            foundry.utils.getProperty(itemChatData, "save.description") !== "None"))
      ) {
        let saveDC = new Roll35e(itemChatData.save.dc.length > 0 ? itemChatData.save.dc : "0", rollData).evaluateSync().total;
