...
export class ItemCombatChanges {
  ...
  hasCombatChange(itemType, rollData) {
    ...
    let combatChangesRollData = foundry.utils.duplicate(rollData);
    ...
    try {
      ...
      return combatChanges.some((change) => {
        return (
          this.#matchesItemType(change["itemType"], itemType) &&
          (change["actionType"] === "" || attackType === change["actionType"]) &&
          (change["condition"] === "" ||
            Roll35e.safeEvaluateCondition(change["condition"], combatChangesRollData))
        );
      });
    } catch {
      return false;
  ...
  getPossibleCombatChanges(itemType, rollData, range = { base: 0, slider1: 0, slider2: 0, slider3: 0 }) {
    ...
    return combatChanges
      ...
      .map((c) => {
        ...
        if (c["formula"].indexOf("d") === -1 && c["field"].indexOf("&") === -1) {
          if (c["formula"] !== "") {
            if (c["formula"].toString().indexOf("@") !== -1) {
              c["formula"] = Roll35e.safeEvaluate(`${c["formula"]}`, combatChangesRollData).total;
            }
            hasAction = true;
          } else {
            c["formula"] = 0;
        ...
        c["applyActionsOnlyOnce"] =
          foundry.utils.getProperty(this.item.system, "combatChangesApplySpecialActionsOnce") ??
          foundry.utils.getProperty(this.item.system, "applyActionsOnlyOnce") ??
          false;
        if (!hasAction) {
  ...
  #matchesItemType(changeItemType, requestedItemType) {
    if (changeItemType === "all") return !requestedItemType.endsWith("Optional");
    if (changeItemType === "allOptional") return requestedItemType.endsWith("Optional");
    return changeItemType === requestedItemType;
  }
}
