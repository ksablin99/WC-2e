...
import { alterRoll, CHAT_MESSAGE_STYLE_KEY, CHAT_MESSAGE_STYLE_OTHER } from "../../lib.js";
...
import { ActorChangesHelper } from "../../actor/helpers/actorChangesHelper.js";
...
export class ItemRolls {
  ...
  async roll(altChatData = {}, tempActor = null) {
    ...
    const template = `systems/warcraftrpg2e/templates/chat/${templateType}-card.html`;
  ...
  async rollAttack(options = {}) {
    ...
    if (!options.data) {
      ...
      rollData.item = foundry.utils.mergeObject(foundry.utils.duplicate(itemData), this.item.getRollData(), { inplace: false });
    } else rollData = options.data;

    // Add CL

    if (this.item.isSpellLike()) {
      ItemSpellHelper.adjustSpellCL(this.item, rollData.item ?? itemData, rollData);
    }
    ...
    if (rollData.item.attackBonus !== "") {
      const attackBonusRoll = await new Roll35e(rollData.item.attackBonus, rollData).roll();
      rollData.item.attackBonus = (attackBonusRoll._total || attackBonusRoll.total || 0).toString();
    ...
    if (options.primaryAttack === false) {
      if (ActorChangesHelper.hasChangeFlag(this.item.actor, "improvedMultiattack")) {
        descriptionParts.push({ name: game.i18n.localize("D35E.AttackSecondaryAttackPenalty"), value: 0 });
      } else if (ActorChangesHelper.hasChangeFlag(this.item.actor, "multiAttack")) {
        parts.push("-2");
    ...
    if (options.bonusOnly) {
      roll = await new Roll35e(parts.join("+"), rollData).roll();
    } else {
      roll = await new Roll35e(["1d20"].concat(parts).join("+"), rollData).roll();
    }
  ...
  async rollEffect({ critical = false, primaryAttack = true } = {}, tempActor = null, _rollData = rollData) {
    ...
    const rollData = foundry.utils.mergeObject(
      foundry.utils.duplicate(actorData),
      {
    ...
    for (let noteObj of noteObjects) {
      ...
      if (noteObj.item != null)
        rollData.item = foundry.utils.mergeObject(foundry.utils.duplicate(noteObj.item.system), noteObj.item.getRollData(), { inplace: false });

      for (let note of noteObj.notes) {
        for (let _note of note.split(/[\n\r]+/)) {
          notes.push(
            await foundry.applications.ux.TextEditor.enrichHTML(`<span class="tag">${Item35E._fillTemplate(_note, rollData)}</span>`, {
              rollData: rollData,
            })
          );
        }
  ...
  async rollDamage({
    ...
    if (damageAttackTypePin != null && damageAttackTypePin !== "") {
      ItemCombatCalculationsHelper.applyWeaponModeDamageMultiplier(
        rollData,
        damageAttackTypePin,
        this.item,
      );
    ...
    // Apply ammo damage parts: "replace" parts substitute weapon base damage, "add" parts stack on top.
    if (ammoDamageParts && ammoDamageParts.length > 0) {
      const replaceParts = ammoDamageParts.filter((p) => p[3] === "replace");
      const addParts = ammoDamageParts.filter((p) => p[3] !== "replace");
      if (replaceParts.length > 0) {
        const weaponParts = parts; // already mapped weapon parts
        const mappedReplace = this.#_mapDamageTypes(replaceParts);
        if (replaceParts.length === 1) {
          // Swap only the first weapon damage part; keep any additional weapon parts after it.
          const rp = mappedReplace[0];
          if (!rp.base) rp.base = weaponParts[0]?.base || "0";
          parts = [rp, ...weaponParts.slice(1)];
        } else {
          // Replace the entire weapon damage list with all replace-mode parts.
          parts = mappedReplace.map((rp, i) => {
            if (!rp.base) rp.base = (weaponParts[i]?.base ?? weaponParts[0]?.base) || "0";
            return rp;
          });
        }
      }
      if (addParts.length > 0) {
        const mappedAdd = this.#_mapDamageTypes(addParts);
        extraParts = [...extraParts, ...mappedAdd.map((p) => [p.base, p.damageType, p.damageTypeUid])];
      }
    }
    ...
    parts = parts.concat(
      extraParts
        ...
        .map((p) => {
          ...
          if (p[2] && p[2] !== null) p[1] = CACHE.DamageTypes.get(p[2])?.name ?? p[1];
          else if (p[1]) {
    ...
    for (let a = 0; a < parts.length; a++) {
      ...
      if (a === 0) {
        ...
        roll = {
          roll: await new Roll35e(rollString, rollData).roll(),
  ...
  async rollAlternativeDamage({ data = null } = {}) {
  ...
  #_mapDamageTypes(damageParts) {
    let parts = damageParts.map((p) => {
      if (p[2]) p[1] = CACHE.DamageTypes.get(p[2])?.name ?? p[1];
      else if (p[1]) {
        for (let damageType of CACHE.DamageTypes.values()) {
