...
import { alterRoll, createTag, getOriginalNameIfExists, getSystemTemplate, linkData, CHAT_MESSAGE_STYLE_KEY, CHAT_MESSAGE_STYLE_CHAT } from "../lib.js";
...
export class Item35E extends ItemBase35E {
  ...
  get intelligentPowers() {
    return this.getExtension("intelligentPowers");
  }
  ...
  get hasAttack() {
    return (
      ["mwak", "rwak", "msak", "rsak"].includes(foundry.utils.getProperty(this.system, "actionType")) ||
      foundry.utils.getProperty(this.system, "actionType") === "summonWeapon"
    );
  }
  ...
  static async setMaxUses(data, rollData) {
    await ItemChargeUpdateHelper.setMaxUses(data, rollData);
  }
  ...
  async update(updated, options = {}) {
    ...
    if (
      ...
      updated["system.containerId"] !== undefined ||
    ...
    if (this.pack && this.pack.startsWith("warcraftrpg2e")) {
    ...
    if (updated["system.convertedWeight"] !== undefined && updated["system.convertedWeight"] !== null) {
      const conversion = game.settings.get("warcraftrpg2e", "units") === "metric" ? 2 : 1;
    ...
    if (updated["system.selectedMaterial"] && updated["system.selectedMaterial"] !== "none") {
      updated["system.material"] = foundry.utils.duplicate(CACHE.Materials.get(updated["system.selectedMaterial"]));
    ...
    await this._updateMaxUses(updated, { srcData: srcData });
    ...
    if (this.actor !== null && !options.massUpdate) {
      if (activateBuff) {
          ...
          if (
            ...
            for (const i of foundry.utils.getProperty(this.system, "shapechange.source.items")) {
              if (
                ...
                let data = foundry.utils.duplicate(i);
        ...
        if (game.combats.active && (this.type === "aura" || this.system.timeline?.enabled)) {
  ...
  _updateCalculateMaxDamageDice(data, rollData) {
    if (
      ...
      let roll = new Roll35e(data["system.maxDamageDiceFormula"], rollData).evaluateSync();
  ...
  _updateCalculateDamagePoolData(data, rollData) {
    ...
    if (rollFormula !== undefined && rollFormula !== null && rollFormula !== "") {
      ...
      try {
        data["system.damagePool.total"] = new Roll35e(rollFormula, rollData).evaluateSync().total;
      } catch (e) {
  ...
  _updateCalculateTimelineData(data, rollData) {
    ...
    if (rollFormula !== undefined && rollFormula !== null && rollFormula !== "") {
      ...
      if (data["system.level"] != null && data["system.level"] !== foundry.utils.getProperty(this.system, "level"))
        rollData.item.level = data["system.level"];
      try {
        data["system.timeline.total"] = new Roll35e(rollFormula.toString(), rollData).evaluateSync().total;
      } catch (e) {
  ...
  async _updateMaxUses(data, { srcData = null, actorData = null, actorRollData = null } = {}) {
    await ItemChargeUpdateHelper.updateMaxUses(this, data, {
      srcData: srcData,
      actorData: actorData,
  ...
  getActorItemRollData() {
    ...
    const rollData = this.actor ? foundry.utils.duplicate(this.actor.getRollData(null, true)) : {};
  ...
  getCombatActor(actor) {
    return actor.isToken
      ? game.combats.active.getCombatantsByToken(actor.token.id)?.[0]
      : game.combats.active.getCombatantsByActor(actor.id)?.[0];
  }
    ...
    for (const c of allCombatChanges) {
      if (c.specialAction && c.specialAction !== "") {
        ...
        if (c.specialActionCondition && c.specialActionCondition !== "") {
          if (!Roll35e.safeEvaluateCondition(c.specialActionCondition, rollData)) continue;
        }
  ...
  async rollFormula(options = {}) {
    ...
    const roll = await new Roll35e(itemData.formula, rollData).roll();
  ...
  async rollConsumable(options = {}) {
    ...
    if (effectStr === "") {
      ...
      // Execute the roll
      let roll = await new Roll35e(parts.join("+"), data).roll();
  ...
  getRollData(customData = null) {
    ...
    const parentRollData = this.parentItem?.getRollData?.();
    if (parentRollData) result.parent = foundry.utils.duplicate(parentRollData);
        ...
        if (foundry.utils.getProperty(this.system, "timeline.elapsed") + time >= foundry.utils.getProperty(this.system, "timeline.total")) {
          if (!foundry.utils.getProperty(this.system, "timeline.deleteOnExpiry")) {
            ...
            if (this.actor.token) {
              ...
              return updateData;
            } else return { id: this.id, delete: true, "system.active": false };
          }
        } else {
          let updateData = {};
  ...
  static async toPolymorphBuff(origData, type) {
    ...
    let data = foundry.utils.duplicate(itemTemplate?.buff ?? {});
  ...
  static async polymorphBuffFromActor(shapechangeData, origData, type) {
    ...
    // Set description
    shapechangeData.system.description.value = await foundry.applications.handlebars.renderTemplate(
  ...
  /**
   * Converts a spell item to an attack item
   * @param {Object} origData - The original item data
   * @param {string} type - The type of attack item (wand, potion, scroll, dorje, tattoo, powerstone)
   * @returns {Object} The converted item data
   */
  static async toAttack(origData, type) {
    const itemTemplate = getSystemTemplate("Item");
    let data = foundry.utils.duplicate(itemTemplate?.attack ?? {});
    for (const t of data.templates ?? []) {
      foundry.utils.mergeObject(data, foundry.utils.duplicate(itemTemplate?.templates?.[t] ?? {}));
    }
    delete data.templates;
    ...
    // Set description
    system.description.value = origData.system.description?.value ?? "";

    return data;
  }

  /**
   * Converts a spell item to a trait item
   * @param {Object} origData - The original item data
   * @param {string} type - The type of trait item (trait, feat, class feature)
   * @returns {Object} The converted item data
   */
  static async toTrait(origData, type) {
    const itemTemplate = getSystemTemplate("Item");
    let data = foundry.utils.duplicate(itemTemplate?.feat ?? {});
    for (const t of data.templates ?? []) {
      foundry.utils.mergeObject(data, foundry.utils.duplicate(itemTemplate?.templates?.[t] ?? {}));
    }
    delete data.templates;
  ...
  async getLinkDataFromData(itemData) {
    const updateData = {};
    let _linkedItems = foundry.utils.duplicate(foundry.utils.getProperty(this.system, `linkedItems`) || []);
    let linkedData = {};
  ...
  async renderBuffEndChatCard() {
    ...
    let chatData = {
      ...
      speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      rollMode: game.settings.get("core", "rollMode"),
      content: await foundry.applications.handlebars.renderTemplate(chatTemplate, { item: this, actor: this.actor }),
    };
    // Handle different roll modes
    switch (chatData.rollMode) {
      case "gmroll":
  ...
  hasUnmetRequirements(rollData) {
    ...
    for (const _requirement of foundry.utils.getProperty(this.system, "requirements") || []) {
      ...
      if (_requirement[2] === "generic") {
        if (!Roll35e.safeEvaluateCondition(_requirement[1], rollData)) {
  ...
  async addSpellToClassSpellbook(level, spell) {
    const updateData = {};
    let _spellbook = foundry.utils.duplicate(this.system?.spellbook || []);
