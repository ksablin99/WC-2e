...
export class CombatD35E extends Combat {
  ...
  async rollInitiative(ids, { formula = null, updateTurn = true, messageOptions = {} } = {}) {
    ...
    for (let id of ids) {
      ...
      let roll;
      try {
        roll = await Roll35e.create(formula, actorData).evaluate();
      } catch (err) {
      ...
      const chatData = foundry.utils.mergeObject(
        {
          ...
          speaker: {
            ...
            token: c.token?.id ?? null,
            alias: c.token?.name ?? c.name,
          },

          flavor: game.i18n.localize("D35E.RollsForInitiative").format(c.token?.name ?? c.name),
          rolls: [roll],
          content: await foundry.applications.handlebars.renderTemplate("systems/warcraftrpg2e/templates/chat/roll-ext.html", rollData),
        },
        messageOptions
      );
    ...
    let buffsToAdd = [];
    if (game.settings.get("warcraftrpg2e", "advanced-combat-tracking")) {
      for (let id of ids) {
        const c = this.combatants.get(id);
        if (!c) continue;
        if (c.actor) {
  ...
  static showInitiativeDialog = function (formula = null) {
    return new Promise((resolve) => {
      const template = "systems/warcraftrpg2e/templates/chat/roll-dialog.hbs";
      ...
      const buttons = {
        normal: {
          ...
          callback: (html) => {
            const dlgRoot = html;
            rollMode = dlgRoot.querySelector('[name="rollMode"]').value;
            const bonus = dlgRoot.querySelector('[name="bonus"]').value;
  ...
  updateCombatCharacterSheet() {
    if (game.settings.get("warcraftrpg2e", "useCombatCharacterSheet")) {
