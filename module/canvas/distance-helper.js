export class DistanceHelper {
  ...
  static _getThreatDistancesInPixels(token) {
    ...
    if (!weapon) {
      if (token.actor.system.traits.reach) distFeet = parseInt(token.actor.system.traits.reach);
    } else if (weapon.system.properties.rch) {
      minDistFeet = 5;
      distFeet = 10;
    }
  ...
  static drawThreatenedHighlights(token) {
    ...
    DistanceHelper.clearThreatHighlights();
    const mode = game.settings.get("warcraftrpg2e", "threatened-display-mode");
    if (mode === "none") return;
