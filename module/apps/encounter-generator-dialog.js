export class EncounterGeneratorDialog extends FormApplication {
    ...
    async loadCompendium() {//is the Bestiary compendium loaded?, No? Load it
        if (game.D35E.compendiumBrowser.type !== 'bestiary' || game.D35E.compendiumBrowser.entityType !== 'Actor' || game.D35E.compendiumBrowser._data.loaded === false) {
            ui.notifications.info("Initializing Encounter Generator...")
            game.D35E.compendiumBrowser.type = 'bestiary';
    ...
    async getTables() {//What Roll tables are our options?
        ...
        for (let table of tables) {
            tableArray.push({ id: table.id, name: table.name })
        }
        return tableArray
    };
...
//Function to get the monsters
    async getMonsters() {
        document.getElementById("putMonstersHere").innerHTML = ""
        //declare stuff
        const grabbedTables = await this.getCompendiumTables();
        let EL = 0
        ...
        let j = 0
        let breakOut = false
        if (grabbedTables.find(t => t.id === val).results.filter(result => result.type !== CONST.TABLE_RESULT_TYPES.DOCUMENT).length !== 0) {
            game.D35E.logger.log(grabbedTables.find(t => t.id === val).results);
        ...
        while (j < limit) {
            ...
            if (EL < targetEL) {
                ...
                while (testCount <= testLimit) {
                    let monsters = (await grabbedTables.find(t => t.id === val).roll()).results
                    testELArray = foundry.utils.duplicate(monsterArray)
                    monsters.forEach(monster => {
                        let monsterCR = game.D35E.compendiumBrowser.items.find(x => x.item.id === monster.documentId)
                        testELArray.push({
...
//shove it all together Monsters name and CR + How many we found
        ...
        async function makeMonsters() {//add all the monster data to the Encounter Gen window (in the form of a pretty little template)
            var newMonster = await foundry.applications.handlebars.renderTemplate("systems/warcraftrpg2e/templates/apps/encounter-generator-template.html", monsterData);
            document.getElementById("putMonstersHere").insertAdjacentHTML("beforeend", newMonster);
        };
        makeMonsters();
...
//Push Result EL to Dialog box html
        document.getElementById("ELGoesHere").textContent = ELText
        document.querySelector(".encounter-block").style.display = ""
    }
    ...
    activateListeners(html) {
        ...
        root.querySelector(".MonsterButton").addEventListener("click", this.getMonsters.bind(this));
    }
