...
export class CompendiumBrowser extends Application {
  ...
  async loadData() {
    const loaderLabel = document.getElementById("d35e-compendium-browser-loader-label");
    const loaderBar = document.getElementById("d35e-compendium-browser-loader-bar");
    const loader = document.getElementById("d35e-compendium-browser-loader");
    if (loaderLabel) loaderLabel.textContent = game.i18n.localize("D35E.LoadingCompendiums");
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/warcraftrpg2e/templates/apps/compendium-browser.html",
  ...
  async _fetchMetadata() {
    ...
    for (let p of game.packs.values()) {
      ...
      if (loaderBar) loaderBar.style.width = `${loadedPercent}%`;
      if (addedItems)
  ...
  _mapItem(pack, item) {
    const result = {
      ...
      issystem: pack.metadata.packageName === "warcraftrpg2e",
      item: {
        id: item.id,
        uuid: item.uuid,
        name: item.name,
    ...
    // Bestiary-specific variables (CR filter population)
    if (this.type === "bestiary") {
      if (!this.extraFilters) {
        this.extraFilters = { "system.details.cr": [] };
      }
      const cr = item.system?.details?.cr;
      if (cr !== undefined && !this.extraFilters["system.details.cr"].includes(cr)) {
        this.extraFilters["system.details.cr"].push(cr);
      }
    }
  ...
  async _render(...args) {
    ...
    this.filterQuery = /.*/;
    const el = this.element?.nodeType === 1 ? this.element : this.element?.[0];
  ...
  activateListeners(html) {
    ...
    root.querySelectorAll(".compendium-browser-tabs .browser").forEach(el => {
      el.addEventListener("click", (e) => this.presetAndLoadFromTab(e));
    });
  }
