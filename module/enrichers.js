...
//@DamageRoll[fire,1d6+22]
    ...
    document.body.addEventListener("click", async (event) => {
      const target = event.target.closest("a.d35e-skill-check");
      if (!target) return;
      event.preventDefault();
      let skill = target.dataset.skill;
      let dc = target.dataset.dc;
      let rollMode = target.dataset.rollMode ?? game.settings.get("core", "rollMode");
    ...
    document.body.addEventListener("click", async (event) => {
      const target = event.target.closest("a.d35e-damage-roll");
      if (!target) return;
      event.preventDefault();
      let damageType = target.dataset.damagetype;
      let damageRoll = target.dataset.damageroll;
      let rawDamageType = target.dataset.rawdamagetype;
      let sourceName = target.dataset.sourceName || "Damage Roll";
      await this.#doDamageRoll(damageRoll, damageType, rawDamageType, sourceName);
    });
    document.body.addEventListener("click", async (event) => {
      const target = event.target.closest("a.d35e-saving-throw");
      if (!target) return;
      event.preventDefault();
      let save = target.dataset.save;
      let dc = target.dataset.dc;
      let rollMode = target.dataset.rollMode ?? game.settings.get('core', 'rollMode');
      game.D35E.requestRoll({
        rollType: 'save',
    ...
    document.body.addEventListener("click", async (event) => {
      const target = event.target.closest("a.d35e-ability-check");
      if (!target) return;
      event.preventDefault();
  ...
  static async #doDamageRoll(damageRoll, damageType, rawDamageType, sourceName) {
    let damage = await new Roll35e(`${damageRoll}`).roll();
    ...
    let chatTemplateData = {
      name: sourceName,
      img: `systems/warcraftrpg2e/icons/actions/unknown.png`,
      [CHAT_MESSAGE_STYLE_KEY]: CHAT_MESSAGE_STYLE_OTHER,
      rollMode: 'public',
    };
    const tooltipWrapper = document.createElement('div');
    tooltipWrapper.innerHTML = await damage.getTooltip();
    const tooltipEl = tooltipWrapper.firstElementChild;
    if (tooltipEl) {
      tooltipEl.insertAdjacentHTML('afterbegin', `<div class="dice-formula">${damage.formula}</div>`);
    }
    const tooltipHtml = tooltipEl ? tooltipEl.outerHTML : '<div class="dice-tooltip dmg-tooltip"></div>';
    const templateData = foundry.utils.mergeObject(
        chatTemplateData,
        {
          flavor: `<img src="systems/warcraftrpg2e/icons/damage-type/${damageIcon}.svg" title="${
              damageName
