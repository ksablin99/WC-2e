...
export default class AbilityTemplate extends MeasuredTemplatePF {
  ...
  static fromItem(item, multiplier = 1, rollData = {}, optionalData = {}) {
    ...
    let baseSize = new Roll35e(`${target.size}`, rollData).evaluateSync().total;
    ...
    switch (templateShape) {
      case "cone": // 5e cone RAW should be 53.13 degrees
        if (game.settings.get("warcraftrpg2e", "measureStyle") === true) templateData.angle = 90;
  ...
  activatePreviewListeners(initialLayer) {
    return new Promise((resolve) => {
      ...
      const pfStyle = game.settings.get("warcraftrpg2e", "measureStyle") === true;
      ...
      handlers.mm = (event) => {
        ...
        const pos = this.layer.getSnappedPoint(center);
        this.document.updateSource({ x: pos.x, y: pos.y });
        this.refresh();
        canvas.app.render();
        moveTime = now;
      };
      ...
      handlers.mw = (event) => {
        ...
        if (event.ctrlKey) {
          ...
          this.document.updateSource({ distance: this.document.distance + delta * -Math.sign(event.deltaY) });
        } else {
          ...
          if (this.document.t === "rect") {
            ...
            this.document.updateSource({ distance: this.document.distance + snap * -Math.sign(event.deltaY) });
          } else {
